...
//
// // 랜덤 수 생성 함수
// function generateRandomEnrollment(baseCount, variation = 0.2) {
//   const minCount = Math.max(0, baseCount * (1 - variation));
//   const maxCount = baseCount * (1 + variation);
//   return Math.floor(Math.random() * (maxCount - minCount + 1)) + minCount;
// }
//
// // 도메인 기반 강좌 매핑
...
//   '프로그래밍': [
//     { name: '자바 프로그래밍 기초', baseCount: 20 },
//     { name: '웹 개발 실무', baseCount: 40 },
//     { name: '백엔드 알고리즘의 이해', baseCount: 20 }
//   ],
//   '데이터 사이언스': [
//     { name: '데이터베이스 입문', baseCount: 10 },
//     { name: '인공지능 개론', baseCount: 10 }
//   ]
// };
//
// // 파일 존재 확인 함수
...
// async function collectCoursePreferenceStatistics() {
//   const now = new Date();
//   const fileName = `statistics-admin-prefer-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}.json`;
//
//   // 도메인별 수강생 데이터 생성
//   const domainEnrollments = Object.entries(COURSE_DOMAINS).map(([domain, courses]) => {
//     const domainCourses = courses.map(course => ({
//       courseId: course.name.replace(/\s/g, ''),
//       courseName: course.name,
//       activeStudents: generateRandomEnrollment(course.baseCount)
//     }));
//
//     return {
//       domain,
...
//   const dummyData = {
//     timestamp: now.toISOString(),
//     total: domainEnrollments.reduce((sum, domain) => sum + domain.total, 0),
//     domainPreferences: domainEnrollments
//   };
//
//   try {
//     const saveFolder = process.env.SAVEFOLDER ?? 'jsons';
...
//         data: dummyData
//       });
//       existingData.current_data = dummyData;
//
...
//
// // 실행 함수
// async function runPreferenceCollector() {
//   try {
//     // 즉시 실행
//     await collectCoursePreferenceStatistics();
//
//     // 1시간마다 반복 실행
//     const ONE_HOUR = 60 * 60 * 1000;
//     setInterval(collectCoursePreferenceStatistics, ONE_HOUR);
//   } catch (error) {
//     console.error('선호도 수집기 실행 중 오류:', error);
//   }
// }
//
// // 프로세스 시작
// runPreferenceCollector();
