...
export async function loadAllData() {
  try {
    ...
    const [facilityResponse, coursesResponse, visitorsResponse, studentPagesResponse, performanceResponse, preferenceResponse] = await Promise.all([
      ...
      fetch(preferencePath)
    ]);

    const [facilityData, coursesData, visitorsData, studentPagesData, performanceData, preferenceData] = await Promise.all([
...
function renderPreferenceChart(data) {
  const chartDom = document.getElementById('preferenceChart');
  if (!chartDom) return;

  const chart = echarts.init(chartDom, 'custom');

  const domains = data.current_data.preferences.domains;
  const domainNames = domains.map(d => d.domainName);
  const courseData = domains.flatMap(domain =>
      domain.courses.map(course => ({
        value: course.activeStudents,
        name: course.courseName,
        groupId: domain.domainName,
        itemStyle: {
          color: course.trend === 'rising' ? '#91cc75' :
              course.trend === 'falling' ? '#ee6666' :
                  '#fac858'
        }
      }))
  );
  ...
  const option = {
    ...
    tooltip: {
      trigger: 'item',
      formatter: function (params) {
        const course = domains
            .flatMap(d => d.courses)
            .find(c => c.courseName === params.name);

        return `${params.name}<br/>
                수강생: ${params.value}명<br/>
                트렌드: ${course?.trend === 'rising' ? '상승' :
            course?.trend === 'falling' ? '하락' : '안정'}`;
      }
    },
    legend: {
      type: 'scroll',
...
export function renderCoursePerformanceChart(performanceData) {
...
export async function initDashboard() {
  ...
  const charts = {
    ...
    preference: renderPreferenceChart(data.preference)
  };
