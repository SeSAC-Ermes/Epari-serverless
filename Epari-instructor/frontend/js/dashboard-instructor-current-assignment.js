async function loadCurrentAssignmentData() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/statistics/current-assignment/${yyyyMMdd}`);
...
async function initCurrentAssignmentDashboard() {
  ...
  const charts = {
    currentAssignmentStatus: renderCurrentAssignmentStatusChart(data.statistics_list[0])  // 수정된 부분
  };
